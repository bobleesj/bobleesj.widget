...
export class WebGPUFFT {
  ...
  async fft2D(
    ...
    const log2Width = Math.log2(width);
    const log2Height = Math.log2(height);

    if (!Number.isInteger(log2Width) || !Number.isInteger(log2Height)) {
      throw new Error('FFT dimensions must be powers of 2');
    }
